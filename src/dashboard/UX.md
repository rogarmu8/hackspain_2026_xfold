...
Nota de integración: este diseño se elaboró antes del cambio a OpenArm, ninja fold y bag chute (commit 0389138). Las etapas y mecanismos aquí descritos son referencias del planteamiento anterior; antes de conectar la UI, adaptar sus etiquetas y métricas a SOLUTION.md y al protocolo vigente. La referencia visual conserva valor estético, no define el robot ni la celda actual.
...
- Etapas: Recogida → Colocación → Prensado → Transferencia → Plegado → Empaquetado. Reinicio se muestra cuando ocurra, separado del resultado productivo. Respetar los estados del protocolo; no saltar PLACE para imitar la imagen.
