...
The line emits `LOAD → TO_PRESS → PRESS → TO_FOLDER → FOLD → INSERT → TO_SEAL → SEAL → TO_CARTON → DONE`. Sub-operations preserve the physical stage (`PRESS/STEAM/LIFT`, each flap, `BAG/TILT/PEEL/RELEASE`, etc.). Bag preparation emits parallel operations on station `bagger`, without replacing the main operation.
