...
| POST | `/runs`, `/batches` | Launch. Run: `name`, `seed`, `scenario`, `clothType` (`tee`… or `random`), `clothCondition` (`good`/`damaged`/`notgood`/`skewed` or `random`). Batch: `count`, `baseSeed`, `clothMix`/`conditionMix` (`same`/`random`/`list`) + `clothTypes`/`conditions`. Catalogue on `GET /capabilities`. |
