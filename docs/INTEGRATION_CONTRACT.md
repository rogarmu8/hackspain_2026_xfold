...
`metric_sample.measurements` contains measured scalar values with units in their keys. Current line measurements: `flatnessPreM` and `flatnessPostM` are standard deviation of vertex z immediately before lowering and after raising the platen; `packLengthM`, `packWidthM`, `packHeightM` are folded AABB dimensions; `foldQualityPct` is OpenCV coverage from the opener-mounted `fold_qc_cam` after the flaps: 100 minus the percent of pack area that is wrinkled *or* sitting off the light folder deck (100 = smooth and fully on the plates). These are observations, not quality gates. Missing measurements stay null; `shirt_in_bag` is nullable and is not inferred from phase. `succeeded` means the line handled this SKU correctly (pack a clean/rotated shirt **into the carton**, or divert stain/tear to the matching tote). A bag that falls off belt 2 but misses the carton is `failed` (`bag missed the carton`), not packed. Seal quality is not validated. `cycleTimeWallS` is elapsed wall time including pauses.
...
| GET | `/runs/{id}/photo` | QC product shot (JPEG; not a journal payload) |
| GET | `/runs/{id}/fold-photo` | OpenCV fold-eval overlay (JPEG; not a journal payload) |
...
| POST | `/runs`, `/batches` | Launch. Run: `name`, `seed`, `scenario`, `clothType` (`tee`… / `custom` or `random`), `clothCondition` (`good`/`damaged`/`notgood`/`skewed` or `random`), optional `conditionMix: "multiple"` + exactly two `conditions` (simultaneous on one garment, e.g. stained + rotated; torn+stained forbidden), optional `clothTypeWeights` / `clothConditionWeights` when random (0 = never). `custom` is a silhouette sheet: send `customDesign` `{ mime, data, outlineUv? }` with Launch (base64 photo; optional PNG-space outline from the dashboard editor). The bridge cuts a flexcomp to that outline (or detects one) and prints both faces; never a journal payload. Batch: `count`, `baseSeed`, `clothMix` (`same`/`random`/`list`) / `conditionMix` (`same`/`random`/`list`/`multiple`) + `clothTypes`/`conditions` + the same weight maps; the photo is required if the mix can draw `custom`. `seed` also draws a skewed heading (flat on the belt). Catalogue on `GET /capabilities`. |
| DELETE | `/runs/{id}` | Remove a run from the catalogue (SQLite + memory + photo/video/trajectory). A live or queued cycle is force-quit first. Cancelled rows are always deletable. Capability `deleteRun`. Journal `run_deleted`. |
