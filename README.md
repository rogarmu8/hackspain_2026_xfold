...
- **Viewer:** full cell (`moon run sim:view`) or shirt playground (`moon run sim:shirt-play`) — **single T panel** the two claws can crease (SOLUTION.md §5.3). A hollow front+back shell looks wearable but the layers slide and the fold becomes a rag; keep that behind `generate_shirt_mesh --shell`. Cloth knobs live in [`src/sim/models/shirt.toml`](src/sim/models/shirt.toml).
...
- **Why the floor used to fail:** MuJoCo still caps **one flex-vs-geom pair** at 50 contacts (`mjMAXCONPAIR`). The whole shirt against one plane is one pair, so only 50 vertices were supported and the rest jittered through the floor — lifting a single point looked fine because almost nothing was touching. `apply_shirt_config` now attaches an invisible sphere to every flex vertex (each sphere is its own pair) and turns off flex-element collision. Density is a drape knob again — keep the default near 400 verts so the playground stays realtime. Regenerate with `python -m xfold.generate_shirt_mesh --spacing 0.032`. Full notes in [SOLUTION.md §4.3](SOLUTION.md#43-cloth--garment-simulation).
- **Ninja landmarks:** `xfold.ninja.landmarks` picks the crease-line vertices (left/right third + hem + collar) for the dual-arm fold in [SOLUTION.md §5.3](SOLUTION.md#station-3--japanese--ninja-fold-two-hands-on-the-press). The playground demo (`N` / orange button) pinches **exactly two claws** at a time — mid + hem on one crease, then the other, then two points on the hem — never a whole-panel force.
- **Cloth tuning gotchas** (all measured on a drop test):
  ...
  - **No `<joint damping>` on the cloth.** It is absolute-frame drag, so it slowed free fall by 1.5x and read as a heavy object in slow motion.
  - `solver="CG"` is ~2x faster than Newton on this flex at the same accuracy. Keep `timestep="0.002"` from `shirt.toml` so the playground stays near realtime.
