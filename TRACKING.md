...
| 2026-09-20 | **Persistencia de experimentos (SQLite).** `data/experiments.sqlite` en la box (y en local): un experimento = un run. Runtime upsert en start/finish + journal events; `GET /experiments` y `GET /runs/{id}` hidratan tras reinicio; vídeo/foto siguen en disco (solo rutas). `GET /experiments/{id}` para detalle. Numeración RUN-/B- continúa desde el máximo persistido. | bridge |
