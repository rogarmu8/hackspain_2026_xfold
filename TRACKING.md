...
| 2026-09-19 | **Fold recipes from the QC photo.** After the flash, OpenCV segments `qc_cam` (rest-mesh fallback) and picks FlipFold, a waist drop-gate pre-fold + FlipFold, or trousers crease + multi-pass cross-folds. New SKU `trousers`. Dress hem narrowed to 0.84 m so the side flaps cover it. Measurements `clothLengthM` / `clothWidthM`. Inlet `flap_prefold` in `line.xml`. | press + cloth |
| | | |
